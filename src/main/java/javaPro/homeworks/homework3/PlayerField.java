...
public class PlayerField extends Field {
    ...
    public void createPlayerField(Players players) {
        for (Ships shipType : Ships.values()) {
            for (int i = 0; i < shipType.numberOfShips; i++) {
                placeTheShip(shipType.numberOfDecks, players);
            }
        }
        repaint();
    }
    ...
    public void placeTheShip(int numberOfDecks, Players player) {
        repaint();
    ...
    public ShotConfirmation takeAShot(Position position) {
        if (field[position.getX()][position.getY()] == DECK_CELL) {
            field[position.getX()][position.getY()] = DAMAGED_DECK;
            if (hasMoreDecks(position)) {
                return ShotConfirmation.DAMAGED;
            } else {
                return ShotConfirmation.KILLED;
            }
        } else {
            return ShotConfirmation.MISSED;
        }
    }
